...
contract('DVideo', ([deployer, author, tipper]) => {
  let dvideo
  ...
  describe('videos', async () => {
    ...
    it('lists videos', async () => {
      ...
      assert.equal(video.tipAmount, '0', 'tipAmount is correct')
    ...
    it('allows users to tip videos', async() => {
      ...
      result = await dvideo.tipVideoOwner(videoCount, { from: tipper, value: web3.utils.toWei('1', 'Ether') })
